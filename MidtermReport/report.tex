...
variables such that $p(y|x)$ is maximized.  Belief propogation is an
exact inference algorithm on tree-structured models.  Approximate
algorithms on models with cycles include loopy belief propogation and
MCMC methods.
...
have a less influence on the author linkage dicision than that over a
...
\begin{figure}
\centering
\includegraphics[height=.55\textheight]{coauthor}
...
The Jaro similarity metric for s and t is given $s= s_1,s_2,s_3...$ and $t= t_1,t_2,t_3...$ it defines a match of $s_i$ to a charactor $t_j$ if $s_i=t_j$ and j is $\frac{min(s,t)}{2} < |i-j|$. If $s'$ is the charactors that match in $s$ and $t'$ are the charactors that match in t, $T$ is half the number of elements that are not exactly matched in position and charactor in $s$ and $t$ and $P$ is the max of longest common prefix of $s$ and $t$ and 4 then Jaro is defined as:
\begin{center}
\[
...
For SoftTFIDF we need a function $CLOSE(\theta; s; s)$ which is defined as the set of words such that there is some $v \in t$ in which $dist(w; v) > \theta$. SoftTFIDF modifys regular cosine similary by summing over all words in $CLOSE(\theta; s; t)$ the product of regular TFIDF of $s_w$, $t_w$, and $dist(w;t)$. Jaro-Winkler SoftTFIDF is implemented by using Jaro-Winkler as the distance function. 

For other fields, such as publication name, venue compatibility, and coauthorship compatibilty, more involved similarity features are neccisary. For coauthorship compatibility we intend to make use of coauthorship graphs. An example coauthorship graph is displayed in Figure \ref{fig:coauthor}. Given an undirected graph $G=(N \cup M,E)$ where $M$ are the entity mentions of a publication, $N$ are the entities assumed to be correctly disambugated based on identical string name, and each $e \in E$ implies coauthorship between the entities. The edges between the mentions being classified are coauthor mentions in the publication. There cannot be any edges between the two entities. We define distance in this graph between the two entites as the shortest path in graph $G$ between the two verticies in $M$. To make such computation efficient, we will set the distance above a certain threshold to be infinity.

We can also define similariy in venues given a herieicary of venues in the field of computer science. The distance will be how far up in the heriecicay one needs to travel to reach a category both venues belong to. 
